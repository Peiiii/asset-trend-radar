...
type DirectorySortMetricDefinition = {
  label: string;
  unit: "percent" | "number" | "money" | "price";
  getValue(item: AssetDirectoryItem): number | null;
};

const directorySortMetrics: Partial<Record<AssetDirectorySortKey, DirectorySortMetricDefinition>> = {
...
export class DirectoryRankingSummaryBuilder {
  public build = (items: AssetDirectoryItem[], totalCount: number, sort: AssetDirectorySortKey, order: AssetDirectorySortOrder): DirectoryRankingSummaryModel => {
    ...
    if (!metric) {
      return {
        label: "排序质量",
        description: `当前按${sort === "label" ? "名称" : "相关性"}排序，列表顺序不代表数值强弱。`,
        badgeLabel: "文本排序",
        badgeTone: "neutral",
        metrics: [
          { label: "本页资产", value: items.length.toLocaleString("en-US"), tone: "blue" },
